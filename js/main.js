  ...
  common: {
    ...
    init: function () {
      // JavaScript to be fired on all pages
        ...
        $( '#mob-trigger' ).unbind('click').click ( function (ev) {
...
// -----------------------------------------------------------
  ...
  let starfield = {
    ...
    updateScreen: function(){
      ...
      let mark = document.getElementById('sp-header');

      
      spaceState.height = (mark.clientHeight + (spaceState.height * .20) );
      spaceState.canvas.height = (mark.clientHeight + (spaceState.height * .20) );
      spaceState.canvas.width = spaceState.width;

      this.intervalb = setInterval(() => draw(), 1000 / spaceState.fps);
    ...
    start: function(){
      ...
      //  Create the stars
        ...
        for(var i=0; i < spaceState.stars; i++) {

          star = {
            x: (Math.random()*spaceState.width), 
                y: (Math.random()*spaceState.height), 
                  size: (Math.random()*3+1),
                velocity: (Math.random()*(spaceState.maxVelocity - spaceState.minVelocity))+spaceState.minVelocity
          }

            stars.push(star);
      }

      this.interval = setInterval(() => draw(), 1000 / spaceState.fps);
   ...
   function draw(){
        ...
        ctx.fillRect(0, 0, spaceState.width, spaceState.height);
     
        //  Draw stars
        ctx.fillStyle = '#ffffff';
        for(var i=0; i<spaceState.stars.length;i++) {
            var star = spaceState.stars[i];
            if(star.size < 3 && star.size > 1.5) {
    ...
    artgallery: {
      ...
      init: function () {
        ...
        $.each ( $D.slideWrapper.children(), function( i, child ) {
          ...
          if(artworkWidth <= 0){
            artworkWidth = ((parseInt($( child ).css('marginRight').replace('px', ''))) * 2) + $( child ).outerWidth();
          }
        ...
        function slideright(){
            ...
            if ( $D.scrollPosition >= ( ( $D.sliderInnerWidth - ( artworkWidth * 2) ) * -1 ) ) {
